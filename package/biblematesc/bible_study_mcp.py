...
from biblematesc import BIBLEMATE_VERSION, BIBLEMATEDATA, AGENTMAKE_CONFIG, config
from biblematesc.uba.bible import search_bible
from biblematesc.uba.api import run_uba_api, run_uba_ai_commentary, run_uba_words, run_uba_discourse, run_uba_translation, run_uba_index
...
if os.path.isfile(dictionary_db):
    ...
    def dictionary(query:str) -> Union[str, list]:
        ...
        from biblematesc.uba.search import UBASearches
        dictionary_db = os.path.join(BIBLEMATEDATA, "dictionary.db")
...
def retrieve_chinese_bible_verses(request:str) -> str:
    """retrieve simplified Chinese Bible 中文和和本聖經 verses; bible verse reference(s) must be given, e.g. John 3:16-17; single or multiple references accepted, e.g. Deut 6:4; Gen 1:26-27"""
    from agentmake.plugins.uba.lib.BibleParser import BibleVerseParser
    refs = BibleVerseParser(True, language="tc").extractAllReferencesReadable(request)
    if not refs:
        return "Please provide a valid Bible reference to complete your request."
...
def retrieve_chinese_bible_chapter(request:str) -> str:
    """retrieve a whole Bible chapter in simplified Chinese 繁體中文; bible chapter reference must be given, e.g. John 3"""
    from agentmake.plugins.uba.lib.BibleParser import BibleVerseParser
    import re
    refs = re.sub("[Cc]hapter ([0-9])", r"\1", request)
    refs = BibleVerseParser(True, language="tc").extractAllReferencesReadable(refs)
